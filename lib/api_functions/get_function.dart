...
import 'package:chat360/screens/chat_room/chat_screen.dart';
...
getChatListResponse({
  ...
  var response = await getChatList(
      ...
      selectedKeys: mainProvider.categoryList,
      context: context);
  if (response.isSuccessful!) {
    if (provider.messageList.length < response.data!.length) {
...
getKeywordListResponse({
  ...
  var response = await getKeywordList(
      searchKeyword: searchKeyword, subCategory: subCategory);
  if (response.isSuccessful!) {
